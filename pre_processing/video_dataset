...
def img_tensor2numpy(tensor_img):
    """将张量图像转换为 NumPy 格式并保证它是 uint8 格式"""
    img = tensor_img.detach().cpu().numpy()  # 将 PyTorch 张量转换为 NumPy 数组
    img = np.transpose(img, (1, 2, 0))  # 将张量维度从 (C, H, W) 转换为 (H, W, C)
    img = (img * 255).astype(np.uint8)  # 转换为 uint8 格式
    return img
